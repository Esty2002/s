...
async function addPriceList(values) {
    ...
    const result = await postData(sqlServer, '/sql/create',
        {
            tableName: 'priceList',
            values: values
        })
...
async function updatePriceList(data) {
    ...
    const result = postData(sqlServer, '/sql/update',
        {
            tableName: 'priceList',
            values: { date: `${setTheDateForSql(data.date)}` },
            condition: `id='${parseInt(data.id)}'`
        })
    return result
}
...
async function deletePriceList(id) {
    ...
    const result = postData(sqlServer, '/sql/delete',
        {
            tableName: 'priceList',
            values: { disable: '0' },
...
async function selectAreaAndPriceByItemCode(condition) {
    const result = await postData(sqlServer, '/sql/readTop20',
        {
            tableName: 'priceList',
            columns: 'areaName,price',
            condition: `itemCode=${condition.code}`
        })
    return result;
...
async function selectProductByAreaName(condition, flag) {
    let result;
    if (flag) {
        result = await postData(sqlServer, '/sql/readTop20',
            {
                tableName: 'priceList',
                columns: 'itemCode,price',
                conditions: `areaName='${condition.area}'`
            })
        ...
        result = await postData(sqlServer, '/sql/readTop20',
            {
                tableName: 'priceList',
                columns: 'itemCode',
                condition: `areaName='${condition.area}'`
            })
...
async function selectAreaByClientOrSupplyCode(condition) {
    ...
    const result = await postData(sqlServer, '/sql/readTop20',
        {
            tableName: 'priceList',
            columns: 'distinct areaName',
            condition: `priceListCode=${condition.code}`
        })
...
async function selectAllAreasByPriceListCodeAndAreaNameAndItemCode(condition) {
    ...
    const result = postData(sqlServer, '/sql/readTop20',
        {
            tableName: 'priceList',
            columns: 'id,priceListCode,areaName,itemCode,price,reduction,primaryAmount,unitOfMeasure',
            condition: `priceListCode='${condition.code}' AND areaName='${condition.area}' AND itemCode=${parseInt(condition.productCode)}`
        })
    return result;
    ...
    const result = await postData(sqlServer, '/sql/readTop20',
        {
            tableName: 'priceList',
            columns: 'itemCode',
            condition: `priceListCode=${condition.code} AND areaName='${condition.areaName}'`
        })
    return result
}
